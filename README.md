...
- 3）因为预训练的参数中已经积累了很多图片学习知识，所以fine-tuning需要冻结一些层的参数，通常只保留用于分类的最后一个FC层；
- 4）期望的fine-tuning后的目标分类（category）需要重新定义。
...
- 1）保存的帧数据会继续用于对YOLO的**继续fine-tuning**，所以保存的帧数据需要尽量符合Fine-tuning的输入PIC的size，即：416x416/640x640/1280x1280;
- 2）帧数据在保存成文件时，需要按照**固定命名格式**命名，并存到指定目录下(date/vendor/file.png)；
- 3）因为前期可能会出现对帧数据的分类不准确的情况，此时需要人工纠正。为了方便纠正，需要在YOLO处理帧数据时，将**帧数据的完整信息**保存到CSV/EXCEL文件中。
对上述**继续fine-tuning**，**固定命名格式**和**帧数据的完整信息**的具体定义如下：


**继续fine-tuning**
有两个方面的考虑：
- 1）开始时YOLO的识别精度可能会较低，所以需要持续提高YOLO的识别精度。为了能够持续提高YOLO的识别精度，需要不断对YOLO做fine-tuning以使其精度和准确度越来越高；
- 2）随着时间的推移，可能需要实现从VIDEO中提取更多种类物品的帧数据，为了使YOLO能够识别更多种类（category），后续也需要对YOLO持续fine-tuning。
...
- 1.一类数据：隐藏在PIC数据处理流程中，根据YOLO和其他方式获取到的PIC数据（称为内部数据，对外隐藏）构建的PIC数据库，用于和User输入的数据做比对和相似度查询；
- 2.二类数据：User主动提供的原始数据（如：PIC，price，size），UserInfo（如：ID，level，）；
- 3.三类数据：在二类数据的基础之上，在程序运行期间生成的数据（如：Comments，TopList）；
...
- 1）数据存储方式：主键（各个表中通用primary key）的定义，表的定义和分类；
- 2）DB的更新机制：什么情况下触发数据库的更新，更新哪些表，如何避免尽量少的更新；
- 3）视图的定义：设计视图的定义，保证DB不断更新数据的情况下，对外提供的API不用改变（API改变的话，会导致前端的代码也会随之改动，引发稳定性危机）；
